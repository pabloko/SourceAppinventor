...
public class Camera extends AndroidNonvisibleComponent
  ...
  /**
   * Takes a picture and Saveto?.
   */
  @SimpleFunction(description = "TakePictureSaveAs Directory= Name directory to save file " +
  "FileName=Name your picture save, Fileformat=Only jpg or bmp or png")
  public void TakePictureSaveAs(String Directory, String FileName, String FileFormat) {
    Date date = new Date();
    String state = Environment.getExternalStorageState();
    ...
    if (Environment.MEDIA_MOUNTED.equals(state)) {
      ...
      ContentValues values = new ContentValues();
      values.put(MediaStore.Images.Media.DATA, imageFile.getPath());
      values.put(MediaStore.Images.Media.MIME_TYPE, "image/" + FileFormat);
      values.put(MediaStore.Images.Media.TITLE, imageFile.getLastPathSegment());
